...
void insert_data(){
  ...
  cout<<"Enter process id: ";
	...
	struct node *flag,*temp=head;
	while(temp->link!=NULL)
	{
		if((temp->end - temp->start + 1) >= size && temp->free!=1 && (temp->end - temp->start + 1 - size < val))
		{
			val=temp->end - temp->start + 1 - size;
			flag=temp;
			found=1;
		}
		temp=temp->link;
	...
	else
	{
		...
		new_node->link=flag->link;
		flag->link=new_node;
		new_node->end=flag->end;
		flag->end=flag->start+size-1;
		new_node->start=flag->end+1;
		flag->proc_id=id;
		flag->free=1;
		new_node->free=0;
		new_node->proc_id='0';
